...
movies_df = movies_df[movies_df['people'] != 0]
...
# cgv 데이터프레임에서 name 컬럼 추출
cgv_movie_names = set(cgv['name'])

# 다른 데이터프레임에서 cgv와 일치하는 name만 선택하여 해당 name의 리뷰 값을 가져와 cgv 데이터프레임에 추가
for df in [daum, megabox, naver]:
    common_names = set(df[df['name'].isin(cgv_movie_names)]['name'])
    for name in common_names:
        cgv_review = cgv.loc[cgv['name'] == name, 'review'].values
        df.loc[df['name'] == name, 'review'] = cgv_review
...
cgv.review = cgv.review.apply(lambda x: [pre(i) for i in x])
cgv.review = cgv.review.apply(lambda x: x[:3])
result_dict = cgv.to_dict(orient='records')
